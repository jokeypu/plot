...
int Shower_function_test()
{
        ...
        for (Int_t ievt = 0; ievt < maxEvtNo; ievt++) {
            ...
            for (int i = 0; i < npoints; i++) {
                ...
                TVector3 pos(x, y, z);
                //if (pos.Mag()<77) continue;
                //if (pos.Mag()<68||pos.Mag()>77) continue;
                if (pos.Mag()>68) continue;
        ...
        TF1 *f11=new TF1("f11","[0]*exp(-1*[1]*x)",0,20);
        f11->SetLineWidth(2);
        f11->SetLineColor(kGreen);
        f11->SetParameters(f->GetParameter(0),f->GetParameter(1));
        
        TF1 *f22=new TF1("f22","[0]*exp(-1*[1]*x)",0,20);
        f22->SetLineWidth(2);
        f22->SetLineColor(kCyan);
        f22->SetParameters(f->GetParameter(2),f->GetParameter(3));
        
        TF1 *f33=new TF1("f33","[0]*exp(-1*[1]*x)",0,20);
        f33->SetLineWidth(2);
        f33->SetLineColor(kViolet);
        f33->SetParameters(f->GetParameter(4),f->GetParameter(5));
