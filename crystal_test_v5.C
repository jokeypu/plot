...
int crystal_test_v5( TString dir_name="Gamma_one_1G" )
{
    int bin1(400),bin2(400),bin3(150);
    float tx(800),ty(600);
    double xmin(0),xmax(0.5),ymin(-0.6),ymax(0.6),zmin(0),zmax(0.1);
...
int Exec(TString dir_name, TH2D *h, Int_t NGamma){
    ...
    for (Int_t ievt = 0; ievt < maxEvtNo; ievt++) {
        ...
        for (int i = 0; i < nhits; i++) {
            ...
            //h->Fill(DD(&Det_Pos, &Cent_pos, 1.25),Eci - Truth_Energy);
            double delta = 0.23*Eci;
            //h->Fill(Truth_Energy,Eci-Truth_Energy+delta);
            h->Fill(Truth_Energy,Eci-Truth_Energy-delta);
