...
int ReadMCTrack_test4()
{
    FairRunAna *fRun = new FairRunAna();
    TFile* file = new TFile("../../data/Gamma_Range/evtcomplete_digi.root");
    FairFileSource* source = new FairFileSource(file,"InputFile");
    FairRootManager* ioman = FairRootManager::Instance();
    ioman->SetSource(source);
    ioman->InitSource();
    
    TFile* f = new TFile("../../data/Gamma_Range/evtcomplete_sim.root");
    ...
    for (Int_t ievt = 0; ievt < maxEvtNo; ievt++) {
        ...
        for (int i = 0; i < nclusters; i++) {
            ...
            for (int j = 0; j < nlnks; j++){
                ...
                //if (EE > 0.2) continue;
                if (r < 67 ||  r > 70 ) continue;
                histxy->Fill(theta,phi);
		if ( theta > 1.02 && theta < 1.52 && phi > -0.18 && phi < 0.18 ){
