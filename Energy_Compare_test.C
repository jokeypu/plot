int Energy_Compare_test()
{
    ...
    //    for (Int_t ievt = 0; ievt < 350; ievt++) {
        ...
        if ( noverlap_M == 1 && noverlap_B == 1 ) {
            ...
            for ( ptr = M_others_energy.begin(); ptr != M_others_energy.end(); ptr++){
                ...
                //hxy1->Fill(M_energy,B_energy-M_energy); //hxy2->Fill(M_energy,B_energy/M_energy);
    ...
    hxy1->GetYaxis()->SetTitle("E_{crystal} - E_{truth} (GeV)");
    ...
    hxy2->GetYaxis()->SetTitle("E_{crystal} - E_{truth} (GeV)");
    ...
    h1->SetLineWidth(2);
    h1->GetXaxis()->SetTitle("E_{crystal} - E_{truth} (GeV)");
    ...
    h2->SetLineWidth(2);
    h2->GetXaxis()->SetTitle("E_{crystal} - E_{truth} (GeV)");
