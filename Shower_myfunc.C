...
Double_t newfunc3(Double_t distance, Double_t angle, const Double_t par){
    if ( angle > 90 && angle <= 180 ) angle = 180 - angle;
    if ( angle > 45 && angle <= 90 ) angle = 90 - angle;
...
int Shower_myfunc(){
    ...
    TH2D* h = new TH2D("h3D","3D",85,rangex_min,rangex_max,85,rangey_min,rangey_max);
    h->SetMarkerStyle(7);
    ...
    for (int i = 0;i<nstepx+1;i++){
        ...
        for (int j = 0;j<nstepy+1;j++){
            h->Fill(d,angle,newfunc3(d,angle,1.25));
