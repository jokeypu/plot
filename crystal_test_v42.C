...
Double_t m(Double_t d, Double_t angle, Double_t par){
    Double_t h = func.func_h(d);
    if (d < 1.2) return h;
    else if (d < 2){
        Double_t a = func.func_a(d);
        Double_t x0 = func.func_x0(d);
        a *= a;
        angle = abs(fmod(angle,45.0) - 45*(((int)(angle/45.0))%2));
        if (angle<x0) return a*angle*angle+h;
        else {cout<< "Angle:" << angle << ", " << a*x0*x0+(a/(45/x0-1))*(x0-45)*(x0-45)-(a/(45/x0-1))*(angle-45)*(angle-45)<<
            "+" << h << endl;return (a*x0*x0+(a/(45/x0-1))*(x0-45)*(x0-45)-(a/(45/x0-1))*(angle-45)*(angle-45))+h;}
        //return h;//-(0.645*d-0.8)/2.0;//-0.11*TMath::Gaus(d,1.7,0.27);
    } else if (d < -2) return h-0.11*TMath::Gaus(d,1.7,0.27);//h+(0.215*d-0.57)/2.0;
    else return exp(-1* par * d);
}

Double_t newfunc2(const TVector3 *DetPos, const TVector3 *Cent, const Double_t par){
...
Double_t newfunc3(const TVector3 *DetPos, const TVector3 *Cent, const Double_t par){
    ...
    if ( distance < 1 ) {
        ...
        return 1/value;*/
        return func.func_h(distance);
    }else return exp(-1* par * distance)-0*(2-distance);  //0.2*
}
...
Double_t rat(const TVector3 *DetPos_i, const TVector3 *DetPos_0, const TVector3 *Cent, const Double_t par) {
    ...
    //else value = newfunc3(DetPos_i, Cent, par)+0*(2-distance);
    value = newfunc2(DetPos_i, Cent, par)/newfunc2(DetPos_0, Cent, par);
    else value = newfunc2(DetPos_i, Cent, par);
...
int crystal_test_v42( TString dir_name="Gamma_one_1G" )
{
    int bin1(400),bin2(400),bin3(150);
    float tx(800),ty(600);
    double xmin(0),xmax(3),ymin(-0.6),ymax(0.6),zmin(0),zmax(0.1);
    ...
    /*TF1* f1=new TF1("f1","[0]*TMath::Gaus(x,[1],[2])",1.3,2);
    f1->SetParameters(0.3,1.7,0.3);
    f1->SetParLimits(0, 0.1, 0.5);
    f1->SetParLimits(1, 1.4, 2);
    f1->SetParLimits(2, 0.1, 0.6);*/
    
    /*TF1* f1=new TF1("f1","[0]*x+[1]",1.6,2);
    f1->SetParameters(-1,-0.8);
    f1->SetParLimits(0, -2, -0.1);
    f1->SetParLimits(1, 0.1, 2);*/
