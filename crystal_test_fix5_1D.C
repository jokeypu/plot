...
Double_t newfunc3(const TVector3 *DetPos, const TVector3 *Cent, const Double_t par){
    ...
    if ( distance < 1.2 ) return func.func_h(distance);
    else if ( distance < 2) return func.func_h(distance)-0.11*TMath::Gaus(distance,1.7,0.27);
...
int crystal_test_fix5_1D( TString dir_name="Gamma_one_1G" )
{
    int bin1(100),bin2(400),bin3(150);
    float tx(800),ty(600);
    //double xmin(0),xmax(3.5),ymin(-0.6),ymax(0.6),zmin(0),zmax(0.1);
    //double xmin(-0.6),xmax(0.6),ymin(0),ymax(1),zmin(0),zmax(0.1);
    double xmin(-0.04),xmax(0.04),ymin(0),ymax(1),zmin(0),zmax(0.1);
    ...
    h1D->GetXaxis()->SetTitle("E_{ci}-E_{truth}");
