...
int Fit_DigiEnergy_test6( Int_t NO_Angle = 7, Double_t Energy = 1.0){
    ...
    std::string file_name = "doc/WorkData_1Gamma_A7_E0.8_OR.txt";
    Energy = 0.8;
    ...
    //TF1* f=new TF1("f1","[0]*sqrt(TMath::Gaus(pow(x,[1]),[3],[2]))",0,distance_cut);
    TF1* f=new TF1("f1","(FABC(x,[0],[1],[2],[3],[4]))",0,distance_cut);
    f->SetParameters(Energy, 2.5, 0.9, 0.7, 3);
    //f->SetParameters(0.8, 1.5, 0.8, 0);
    /*f->SetParLimits(0, 0.7, 1);
     f->SetParLimits(1, 1, 3);
     f->SetParLimits(2, 0, 1.5);
     f->SetParLimits(3, -0.1, 0.1);*/
